...
def test_predict_returns_valid_probability(client_with_model):
    ...
    # Every response must carry a unique request_id for log correlation
    assert "request_id" in body
    assert len(body["request_id"]) == 36  # standard UUID4 format


def test_predict_request_ids_are_unique(client_with_model):
    """Two calls must produce different request_ids — validates UUID generation."""
    payload = {"engine_id": 1, "readings": _sample_readings(6)}
    id1 = client_with_model.post("/predict", json=payload).json()["request_id"]
    id2 = client_with_model.post("/predict", json=payload).json()["request_id"]
    assert id1 != id2
